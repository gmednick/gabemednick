...
tags: []
subtitle: ''
summary: Web scraping to generate custom data for crypto analysis
authors: []
lastmod: ''
featured: no
image:
...
slug: crypto
---
In this post, I use the httr package to access bitcoin's price chart from [CoinGecko's](https://www.coingecko.com/en) crypto market [API](https://www.coingecko.com/en/api). The rvest package is used to scrape the hourly percentage change for the top 100 cryptos (by market cap) from CG's website. The cron task scheduler (macOS) is then set to run the web scraping script every 15 minutes and write the data to a spread sheet, giving me the percentage change on 15 minute intervals.
