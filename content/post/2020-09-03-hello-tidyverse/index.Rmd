...
A great place to start any analysis is by grouping and counting predictive features using the `count()` function. It often makes sense to combine `count()` with a bar plot to visualize meaningful distributions. Spending time with new data is a worthwhile investment since it lead us to ask better questions and generate ideas that can then be explored using the predictive power of machine learning. 
...
Next, let's make a regression model for each country and crop--we'll use the top 20 countries and the four crops to create 800 models.
```{r}
# using tidymodels ..
