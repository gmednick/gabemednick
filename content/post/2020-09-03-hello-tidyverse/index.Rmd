...
---
# Intro 
This post uses data from the <i class='fab fa-r-project'  style='color: #367588'></i> for Data Science online learning community. Each Tuesday, a dataset is posted and the tidyverse community of data science share their analyses and visualizations on Twitter with the hashtag, #TidyTuesday. This week's dataset (September 1st, 2020) is from [Our World in Data] (https://ourworldindata.org/crop-yields). There are actually five dataset but we are going to focus on global crop yields. 
...
Let's start by downloading the data from the [R4DS Github](https://github.com/rfordatascience/tidytuesday) account and take a look at it. I recommend the tidytuesdayR package (by Ellis Hughes, co-host of the screencast 'TidyX') for importing the data. The package is handy for checking out available datasets, exploring the data dictionary in the Rstudio viewer and loading the desired data. Although simple to use, I'm going to stick with the `readr::read_csv()` from the tidyverse and import the data as a tibble (data frame).
...
Let's start with a little data cleaning! `skim` from `skimr` provides a convenient summary. By looking at the data we can see that there are missing values that need to be addressed. The variable names could be improved and we should always consider the shape of our data. We probably want the data in a tidy format where each column is a feature and each row is a sample.This long format facilitates fluent analysis with the tidyverse. The function `pivot_longer()` from the tidyr package allows us to reshape our data and prepare it for exploratory data analysis (EDA).   
```{r}
...
A great place to start an analysis is by grouping and counting features using the `count()` function.
```{r}
crop_yield_tidy %>%
