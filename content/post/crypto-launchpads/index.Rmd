...
tags: []
subtitle: ''
summary: Visualizing market dynamics for crypto launchpads and accessing twitter data for natural language processing. 
authors: []
lastmod: ''
featured: no
image:
...
This post uses market data from Coingecko's API to explore several launchpad cryptocurrency companies. These launchpads partner with new projects to help them launch into the crypto space by providing a mechanism for fundraising and exposure to the launchpad's connections and community of token holders. In turn, the token holders benefit from early investment access to these new projects. 

After providing basic visual analysis of several launchpads, I will take a deeper dive into TrustSwap. I will webscrape the company's roadmap from their [website](https://trustswap.org) and also scrape pertinent details from [etherscan](https://etherscan.io/token/0xcc4304a31d09258b0029ea7fe63d032f52e44efe). We will then use the rtweets package to explore the most frequent words that are used in tweets that reference the the [@TrustSwap](https://twitter.com/TrustSwap) Twitter handle. Let's get started!
...
Here is a table showing the launchpad projects to query for market data via CoinGecko's API.
...
Let's get the data take a look at the data.
...
# Zooming in on TrustSwap
<center>
![](https://assets.coingecko.com/coins/images/11795/large/Trustswap.png?1594311216)
</center>

Let's start by rendering the historical price data as an interactive Plotly chart that allows the user to interact with the data by scrolling over the plot. 
...
A one-line description of TrustSwap is available on their Medium page. They are more than just a launchpad but we need to did deeper to find out what services they offer.
...
TrustSwap's website does not have a description but rather, they provide a list of services. Scrolling down there is a description of each service, which are essentially stand-alone projects that complement each other. 
...
Let's do a little more webscraping and see what TrustSwap has up its sleeves. 
```{r}
...
TrustSwap's basic tokenomics can be found on etherscan.io -- The Ethereum Blockchain Explorer 
...
# Searching Twitter for tweets that tag @trustswap
...
Now that we've converted the tweets into 'tokens' and removed stop words, we can count the tokens and display the most frequently used words as a bar graph.
...
Another convenient way to view the most common words is as a word cloud where word frequency is represented by size.
