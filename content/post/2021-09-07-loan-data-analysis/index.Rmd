...
This [dataset](https://www.kaggle.com/itssuru/loan-data) consists of data from almost 10,000 borrowers that took loans with some paid back and others still in progress. It was extracted from lendingclub.com which is an organization that connects borrowers with investors.
...
The `credit_policy` and `not_fully_paid` variables (not shown above) are better represented as factors. They can be converted using the `recode_factor()` function. Let's convert them to factors and explore their distributions with `geom_boxplot()`.
...
Loan type can also be viewed with `geom_boxplot()` but the categories get a little crowded.
...
The `ggpairs()` function from the GGally package allows us to summarize correlation values between our variables.
...
For the next two models, I will use xgboost.
